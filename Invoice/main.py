...
class Window1(tk.Toplevel):
    def __init__(self,parent):
        ...
        #--------------------------- FUNCTIONS ----------------------------------
        def default_bill():
            display_txt.insert(END,"\t\t\t            Starbucks")
            display_txt.insert(END,"\n\t\t\t        6th Avenue Street")
            display_txt.insert(END,"\n\t\t\t   Contact- +919456387601")
            display_txt.insert(END,"\n===========================================================================================================")
            display_txt.insert(END,f"\nInvoice ID: {inv_id_ent.get()}")
            display_txt.insert(END,f"\nTime : {date_pr.get()}")


        def gen_bill():
            display_txt.insert(END,f"\nCustomer Name: {cust_name.get()}")
            display_txt.insert(END,f"\nCustomer Contact: {cust_ct.get()}")
            display_txt.insert(END,"\n===========================================================================================================")
            display_txt.insert(END,"Product Name\t\t                     \t\t Quantity                \t\tPrice             \t\t       Total")
            display_txt.insert(END,"\n===========================================================================================================")


        # def add_prd():
            # qty=prd_qty_ent.get()
            # prdid_data=prd_id_ent.get()
            # c.execute("SELECT One_price FROM products WHERE Product_id = %s", (prdid_data,))
            ...
            # price = price_row[0]
            # total = qty * price
            # c.execute("SELECT Product_name FROM products WHERE Product_id = %s", (prdid_data,))
            # prdname_row = c.fetchone()
        ...
        def add_prd():
            ...
            prdname = prdname_row[0]
            total_list.append(total)
            products.append({'product_id': prdid_data, 'quantity': qty})
            display_txt.insert(END, f"\n{prdname}\t\t                     \t\t{qty}                    \t\t{price}              \t\t\t\t      Rs. {total}")
            # def commit():
        #     for product in products:
        #         invid = inv_id_ent.get()
        #         cust_name_data = cust_name.get()
        #         cust_ct_data = cust_ct.get()
        #         qty = product['quantity']
        #         prdid_data = product['product_id']

        #         # c.execute("SELECT Customer_contact FROM invoice WHERE Customer_contact = %s", (cust_ct_data,))
        #         # existing_contact = c.fetchone()

        #         # if existing_contact is not None:
        #         #     messagebox.showerror("Error", "Customer contact already exists in the invoice table")
        #         #     return
        ...
        def total(self):
            ...
            for item in total_list:
                grd_total=grd_total+item
            display_txt.insert(END,"\n===========================================================================================================")
            display_txt.insert(END,f"\t\t\t\t Grand Total :{grd_total}")
            display_txt.insert(END,"\n===========================================================================================================")
        ...
        display_txt = Text(bill_frame, bg="white", height=32, width=107, bd=5,relief=FLAT)
        ...
        Cust_name_entry = Entry(detframe,textvariable=cust_name)
        ...
        Cust_ct_entry = Entry(detframe,textvariable=cust_ct)
        ...
        prd_id_entry = Entry(detframe,textvariable=prd_id_ent)
        ...
        prd_qty_entry = Entry(detframe,textvariable=prd_qty_ent)
...
image_path=r"C:\Users\nsidd\OneDrive\Desktop\Files\Coding\Invoice\image.png"
